...
input_list = [line.strip()
              for line in text_datei if not line.startswith("#") and line != " "]
...
def mergesort(input_list):
    ...
    # Aufteilung rechts links und sortieren
...
def merge(left, right):
    ...
    while i < len(left) and j < len(right):
        ...
        if comp(left[i], right[j]):
            # linkes Element in Rueckgabeliste einfuegen
            result.append(left[i])
            i += 1
        else:
            # rechtes Element in Rueckgabeliste einfuegen
...
def partition(input_list, unten, oben):
    ...
    for j in range(unten, oben):
        if comp(input_list[j], pivot):
            ...
            # tausche elemente <= pivot an stelle <= i
            (input_list[i], input_list[j]) = (input_list[j], input_list[i])
    # tausche pivot an die passende stelle hinter i
    (input_list[i + 1], input_list[oben]
     ) = (input_list[oben], input_list[i + 1])
    return i + 1
